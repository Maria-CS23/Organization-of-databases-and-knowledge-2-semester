...
class PhoneShopApp(QMainWindow):
    ...
    def add_phone(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            try:
                ...
                cursor.execute("SELECT COUNT(*) FROM Phone WHERE PhoneID = ?", phone_data[0])
                ...
                self.load_data()
                QMessageBox.information(self, "Успіх", "Телефон успішно доданий до бази даних!")

            except Exception as e:
                QMessageBox.critical(self, "Помилка", f"Не вдалося додати телефон до БД: {str(e)}")
    ...
    def edit_phone(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            try:
                ...
                cursor.execute("""
                    UPDATE Phone 
                    SET Manufacturer = ?, Model = ?, Specifications = ?, Price = ?, Availability = ?
                    WHERE PhoneID = ?
                """, (
                    updated_data[1],
                    updated_data[2],
                    updated_data[3],
                    float(updated_data[4]),
                    updated_data[5],
                    updated_data[0]
                ))
                ...
                self.load_data()
                QMessageBox.information(self, "Успіх", "Телефон успішно оновлено у базі даних!")

            except Exception as e:
                QMessageBox.critical(self, "Помилка", f"Не вдалося оновити телефон у БД: {str(e)}")
    ...
    def delete_phone(self):
        ...
        confirm = QMessageBox.question(
            self,
            "Підтвердження",
            f"Ви впевнені, що бажаєте видалити телефон {phone_model} (ID: {phone_id}) із бази даних?",
        ...
        if confirm == QMessageBox.Yes:
            try:
                ...
                cursor.execute("SELECT COUNT(*) FROM OrderLine WHERE PhoneID = ?", phone_id)
                if cursor.fetchone()[0] > 0:
                    QMessageBox.warning(self, "Попередження",
                                        "Не можна видалити телефон, оскільки він використовується у замовленнях!")
                    conn.close()
                    return

                cursor.execute("SELECT COUNT(*) FROM Inventory WHERE PhoneID = ?", phone_id)
                if cursor.fetchone()[0] > 0:
                    QMessageBox.warning(self, "Попередження",
                                        "Неможливо видалити телефон, оскільки він використовується в інвентарі!")
                    conn.close()
                    return
                ...
                self.load_data()
                QMessageBox.information(self, "Успіх", "Телефон успішно видалено з бази даних!")

            except Exception as e:
                QMessageBox.critical(self, "Помилка", f"Не вдалося видалити телефон із БД: {str(e)}")
